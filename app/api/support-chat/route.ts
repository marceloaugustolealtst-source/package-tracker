...
export async function POST(req:Request){
  try{
    ...
    if(action==='ensure'){
      ...
      let {data:conversation,error}=await supabase.from('support_conversations').select('*').eq('session_id',sessionId).maybeSingle()
    ...
    if(action==='messages'){
      ...
      if(messages.error) throw messages.error
      const conversation=await supabase.from('support_conversations').select('*').eq('id',conversationId).single()
    ...
    if(action==='send'||action==='bot'){
      ...
      if(!conversationId||!text) return NextResponse.json({error:'Missing message data'},{status:400})
      const conversation=await supabase.from('support_conversations').select('id,session_id').eq('id',conversationId).single()
      if(conversation.error) throw conversation.error
      const inserted=await supabase.from('support_messages').insert({conversation_id:conversationId,sender:action==='bot'?'bot':'customer',body:text}).select('id,sender,body,created_at').single()
      if(inserted.error) throw inserted.error
      if(action==='send'){
        const now=new Date().toISOString()
        const updated=await supabase.from('support_conversations').update({updated_at:now,last_customer_message_at:now}).eq('id',conversationId)
