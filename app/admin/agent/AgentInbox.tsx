...
export default function AgentInbox({conversations,sendAgentMessage,setConversationAutomation,closeConversation}:{conversations:Conversation[];sendAgentMessage:Action;setConversationAutomation:Action;closeConversation:Action}){
  ...
  useEffect(()=>{const supabase=createClient();const reload=()=>{if(!sendingRef.current)router.refresh()};const channel=supabase.channel('agent-live-inbox').on('postgres_changes',{event:'*',schema:'public',table:'support_messages'},()=>reload()).on('postgres_changes',{event:'*',schema:'public',table:'support_conversations'},()=>reload()).subscribe();return()=>{supabase.removeChannel(channel)}},[router])
